...
{
    //打开txt文件（若不存在则新建文件）
    //依次写入bag的成员变量，map要用字符串表示
    //依次写入player成员变量, 指针型成员变量改为存其对应对象的getname()
    //依次写入game成员变量stage, partener_alive, enemy_alive
...
void Game::process(string msg)//根据pos，处理msg文本,最关键的部分，根据地点，划分出小状态
{
    ...
    if(this->stage==NOT_BEGIN)
    {
        ...
        {
            cout<<"读取存档功能尚未实现！"<<endl;
            return;
        }
        else
        {
            cout<<"请输入begin开始游戏，或输入load读取存档！"<<endl;
            return;
        }
    }
    //任何场景下都可执行的指令
